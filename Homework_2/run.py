...
def plot_mw(run_info_dict,layer_name):
    w = run_info_dict['mw'][layer_name]
    w = w/w[0]
    plt.xlabel('epoch')
    plt.ylabel('weights')
    plt.plot(w,label=layer_name)
    plt.title("Weights of linear layers")
    plt.legend()
...
def experiment_MNIST():
    ...
    plot_convergence(run_info)
    plt.show()
    plt.savefig('Mnist-stable.png')
    plt.clf()

    for layer in net.layers:
        if layer.has_params():
            plot_mw(run_info,layer.name)
            
    plt.show()
    plt.savefig(f'norm_weights.png')
    plt.clf()
